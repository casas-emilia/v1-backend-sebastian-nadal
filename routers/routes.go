...
import (
	"time"
	"v1-backend-sebastian-nadal/controllers"
	"v1-backend-sebastian-nadal/middlewares"
...
func SetupRouter() *gin.Engine {
	...
	router.Use(cors.New(cors.Config{
		AllowOrigins: []string{
			...
			"https://v1-backend-casas-charlotte-production.up.railway.app",
			"https://v1-frontend-casas-charlotte-production.up.railway.app",
		}, // Dominios permitidos
	...
	empresas := router.Group("/empresas")
	{
		...
		prefabricadas := empresas.Group("/:empresaID/prefabricadas")
		{
			...
			precios := prefabricadas.Group("/:prefabricadaID/precios")
			{
				...
				adicionales := precios.Group("/:precioID/adicionales")
				{
					adicionales.GET("/", controllers.ObtenerAdicionales)           // Obtener todos los Adicionales de un Precio
					adicionales.GET("/:adicionalID", controllers.ObtenerAdicional) // Obtener Adicional de acuerdo a su ID
				}
			}
		}
	...
	admin := router.Group("/administracion", middlewares.AuthMiddleware(), middlewares.AdminMiddleware())
	{
		...
		{
			empresas.POST("/", middlewares.SuperAdminMiddleware(), controllers.CrearEmpresa)                // Crear Empresa
			empresas.GET("/", controllers.ObtenerEmpresas)                                                  // Obtener todas las Empresas
			empresas.GET("/:empresaID", controllers.ObtenerEmpresa)                                         // Obtener datos de Empresa de acuerdo a su ID
			empresas.PUT("/:empresaID", middlewares.SuperAdminMiddleware(), controllers.ActualizarEmpresa)  // Actualizar datos de Empresa
			empresas.DELETE("/:empresaID", middlewares.SuperAdminMiddleware(), controllers.EliminarEmpresa) // Eliminar Empresa de acuerdo a su ID
			...
			prefabricadas := empresas.Group("/:empresaID/prefabricadas")
			{
				...
				precios := prefabricadas.Group("/:prefabricadaID/precios")
				{
					...
					adicionales := precios.Group("/:precioID/adicionales")
					{
						adicionales.POST("/", controllers.CrearAdicional)                  // Crear un Incluye de un precio
						adicionales.GET("/", controllers.ObtenerAdicionales)               // Obtener todos los Adicionales de un Precio
						adicionales.GET("/:adicionalID", controllers.ObtenerAdicional)     // Obtener Adicional de acuerdo a su ID
						adicionales.PUT("/:adicionalID", controllers.ActualizarAdicional)  // Actualizar Adicional
						adicionales.DELETE("/:adicionalID", controllers.EliminarAdicional) // Eliminar lógicamente un Adicional
					}
				}
			}
			...
			usuarios := empresas.Group("/:empresaID/usuarios")
			{
				usuarios.POST("/", middlewares.SuperAdminMiddleware(), controllers.CrearUsuario)                // Crear un nuevo usuarios
				usuarios.GET("/", controllers.ObtenerUsuarios)                                                  // Obtener todos los Usuarios de una Empresa
				usuarios.GET("/:usuarioID", controllers.ObtenerUsuario)                                         // Obtener Usuario de acuerdo a su ID
				usuarios.PUT("/:usuarioID", controllers.ActualizarUsuario)                                      // Actualizar datos de Usuario de acuerdo a su ID
				usuarios.DELETE("/:usuarioID", middlewares.SuperAdminMiddleware(), controllers.EliminarUsuario) // Eliminar Usuario lógicamente de acuerdo a su ID
		...
		{
			roles.POST("/", middlewares.SuperAdminMiddleware(), controllers.CrearRol)            // Crea un nuevo Rol
			roles.GET("/", controllers.ObtenerRoles)                                             // Obtener todos los Roles
			roles.GET("/:rolID", controllers.ObtenerRol)                                         // Obtener Rol de acuerdo a su ID
			roles.PUT("/:rolID", middlewares.SuperAdminMiddleware(), controllers.ActualizarRol)  // Actualizar datos de un Rol
			roles.DELETE("/:rolID", middlewares.SuperAdminMiddleware(), controllers.EliminarRol) // Eliminar Rol lógicamente

			rolesUsuarios := roles.Group("/:rolID/roles_usuarios")
			{
				rolesUsuarios.POST("/", middlewares.SuperAdminMiddleware(), controllers.CrearRol_usuario)                    // Asignarle un Rol a un Usuario
				rolesUsuarios.GET("/", controllers.ObtenerRoles_usuarios)                                                    // Obtener todos los roles y sus usuarios
				rolesUsuarios.GET("/:rol_usuarioID", controllers.ObtenerRol_usuario)                                         // Obtener Rol_usuario
				rolesUsuarios.PUT("/:rol_usuarioID", middlewares.SuperAdminMiddleware(), controllers.ActualizarRol_usuario)  // Actualizar datos Rol_usuario
				rolesUsuarios.DELETE("/:rol_usuarioID", middlewares.SuperAdminMiddleware(), controllers.EliminarRol_usuario) // Eliminar lógicamente un rol_usuario
			}
		}
	}
