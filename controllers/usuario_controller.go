...
import (
	...
	"v1-backend-sebastian-nadal/services"
...
func ObtenerUsuarios(c *gin.Context) {
	...
	if err := configs.DB.
		Preload("Rol_usuario", func(db *gorm.DB) *gorm.DB {
			return db.Where("deleted_at IS NULL") // Filtra roles no eliminados
		}).
		Preload("Rol_usuario.Rol", func(db *gorm.DB) *gorm.DB {
			return db.Where("roles.deleted_at IS NULL") // Filtra roles no eliminados
		}).
	...
	for _, usuario := range usuarios {
		...
		var rolesResponse []dto.Rol_usuarioResponse
		for _, rol := range usuario.Rol_usuario {
			rolesResponse = append(rolesResponse, dto.Rol_usuarioResponse{
				ID:        rol.RolID,
				CreatedAt: rol.CreatedAt,
				UpdatedAt: rol.UpdatedAt,
				UsuarioID: rol.UsuarioID,
				RolID:     rol.RolID,
				RolNombre: rol.Rol.NombreRol,
			})
		}
...
func ObtenerUsuario(c *gin.Context) {
	...
	if err := configs.DB.
		// Preload("Rol_usuario", func(db *gorm.DB) *gorm.DB {
		// 	return db.Where("deleted_at IS NULL") // Filtra roles_usuarios no eliminados
		// }).
		Preload("Rol_usuario", func(db *gorm.DB) *gorm.DB {
			return db.Where("deleted_at IS NULL") // Filtra roles no eliminados
		}).
		...
		Where("empresa_id = ?", empresaID).
		Where("deleted_at IS NULL").
		First(&usuario, usuarioID).Error; err != nil {
