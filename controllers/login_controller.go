...
func Login(c *gin.Context) {
	...
	if err := configs.DB.
		...
		Preload("Credencial", func(db *gorm.DB) *gorm.DB {
			return db.Where("deleted_at IS NULL") // Condición para no cargar Credencial eliminadas lógicamente
		}).
		Preload("Rol_usuario", func(db *gorm.DB) *gorm.DB {
			return db.Where("roles_usuarios.deleted_at IS NULL") // Filtra los "roles" no eliminados
		}).
		// Preload("Rol_usuario.Rol"). // Precargar Rol_usuario y Rol
		Preload("Rol_usuario.Rol", func(db *gorm.DB) *gorm.DB {
			return db.Where("roles.deleted_at IS NULL") // Filtra los "roles" no eliminados
		}).
