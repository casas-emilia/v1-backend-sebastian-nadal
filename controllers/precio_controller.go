...
func ObtenerPrecios(c *gin.Context) {
	...
	if err := configs.DB.
		...
		Preload("Adicional", func(db *gorm.DB) *gorm.DB {
			return db.Where("deleted_at IS NULL") // Condición para no cargar "adicional" eliminados lógicamente
		}).
	...
	for _, precio := range precios {
		...
		for _, adicional := range precio.Adicional {
...
func ObtenerPrecio(c *gin.Context) {
	...
	if err := configs.DB.
		...
		Where("prefabricada_id = ?", prefabricadaID).Where("deleted_at IS NULL").First(&precio, precioID).Error; err != nil {
